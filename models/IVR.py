...
class IVR(nn.Module):
    ...
    def train_forward_closed(self, x):
        ...
        img = self.global_pool(img)  # (B, C, 1, 1)
        img = img.view(img.size(0), -1)  # (B, C)
        pos_attr_img = self.global_pool(pos_attr_img).view(pos_attr_img.size(0), -1)
        pos_obj_img = self.global_pool(pos_obj_img).view(pos_obj_img.size(0), -1)
        ...
        loss_grad_attr = 1 / 2 * (attr_grads[0][0] - attr_grads[1][0]).pow(2).sum() + 1 / 2 * (
                    attr_grads[0][1] - attr_grads[1][1]).pow(2).sum()
        ...
        loss_grad_obj = 1 / 2 * (obj_grads[0][0] - obj_grads[1][0]).pow(2).sum() + 1 / 2 * (
                    obj_grads[0][1] - obj_grads[1][1]).pow(2).sum()
    ...
    def val_forward(self, x):
        img = x[0]
        img = self.global_pool(img).view(img.size(0), -1)
        img = self.image_embedder(img)
        ...
        for itr, (attr, obj) in enumerate(self.dset.pairs):
            ...
            scores[(attr, obj)] = attr_pred[:, attr_id] * obj_pred[:, obj_id] + pair_pred[:,
                                                                                self.dset.all_pair2idx[(attr, obj)]]
        return None, scores
